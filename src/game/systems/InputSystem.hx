...
class InputSystem implements ISystem {
	...
	public function update(world:World, dt:Float):Void {
		...
		for (e in world.query(PlayerControlled)) {
			...
			var fc = e.get(Facing);
			if (fc != null) {
				// Horizontal movement overrides vertical for facing (feels responsive).
				if (dx != 0) fc.set(dx > 0 ? 1 : -1, 0);
				else if (dy != 0) fc.set(0, dy > 0 ? 1 : -1);
				// No input → keep previous facing.
			}
		}
	}
}
